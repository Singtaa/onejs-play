 ...
 * has to hand.
 *
 * Copied verbatim from Falling Blocks, for the reason given at the top of
 * blocks.ts: a game on this site is built from the files beside it, so reaching
 * into a sibling example would not resolve. The numbers were tuned against a
 * real phone and there is nothing about a versus game that wants them changed,
 * so they are left exactly as they were rather than drifting apart for no
 * reason.
 */
