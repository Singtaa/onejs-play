...
import {
    ...
    scoreFor, levelFor, dropInterval, cellsOf, shapeOf, sevenBag, KINDS, COLS, ROWS,
} from "./blocks"
...
describe("gravity in a fight", () => {
    it("stops accelerating while the game is still playable", () => {
        ...
        for (let level = 0; level < 40; level++) {
            expect(dropInterval(level)).toBeGreaterThanOrEqual(0.15)
        }
    })

    it("still gets faster over the levels a player will actually reach", () => {
        expect(dropInterval(5)).toBeLessThan(dropInterval(0))
        expect(dropInterval(9)).toBeLessThan(dropInterval(5))
    })

    it("never speeds up as the level climbs", () => {
        for (let level = 1; level < 40; level++) {
            expect(dropInterval(level)).toBeLessThanOrEqual(dropInterval(level - 1))
        }
    })
})
...
describe("the piece bag", () => {
    ...
    it("gives all seven kinds before repeating any of them", () => {
        const draw = sevenBag(seeded(5))
        for (let round = 0; round < 50; round++) {
            const seen = new Set(Array.from({ length: 7 }, draw))
    ...
    it("never leaves a player waiting longer than two bags for a kind", () => {
        ...
        for (let i = 0; i < 7000; i++) {
            ...
            if (previous !== undefined) expect(i - previous).toBeLessThanOrEqual(13)
            lastSeen.set(kind, i)
        }
        expect(lastSeen.size).toBe(KINDS.length)
    })

    it("does not always shuffle to the same order", () => {
        const draw = sevenBag(seeded(29))
        const first = Array.from({ length: 7 }, draw).join("")
        const orders = new Set<string>()
        for (let round = 0; round < 40; round++) orders.add(Array.from({ length: 7 }, draw).join(""))
        expect(orders.size).toBeGreaterThan(1)
        expect(orders.has(first) && orders.size === 1).toBe(false)
    })

    it("repeats exactly from the same source, which is what makes it testable", () => {
        const a = sevenBag(seeded(101))
        const b = sevenBag(seeded(101))
        expect(Array.from({ length: 40 }, a)).toEqual(Array.from({ length: 40 }, b))
    })

    it("only ever hands out real kinds", () => {
        const draw = sevenBag(Math.random)
        for (let i = 0; i < 700; i++) expect(KINDS).toContain(draw())
    })
})
